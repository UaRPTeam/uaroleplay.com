...
import { getHomePageSingleton } from "../lib/sanitySingletons";
...
const homeBodyComponents: PortableTextComponents = {
  block: {
    ...
    h2: ({ children, value }) => {
      const textAlign = getBlockAlign(value as { children?: Array<{ marks?: string[]; text?: string }> });
      return (
        <h2 className="mb-4 text-3xl font-bold text-slate-950 sm:text-4xl" style={textAlign ? { textAlign } : undefined}>
          {children}
        </h2>
      );
    },
    h3: ({ children, value }) => {
      const textAlign = getBlockAlign(value as { children?: Array<{ marks?: string[]; text?: string }> });
      return (
        <h3 className="mb-4 text-2xl font-bold text-slate-950 sm:text-3xl" style={textAlign ? { textAlign } : undefined}>
          {children}
        </h3>
      );
    },
...
export default async function Home() {
  const page = await getHomePageSingleton();
  ...
  return (
    <>
      ...
      <section className="relative min-h-[70vh] w-full overflow-hidden rounded-none border border-white/60 bg-transparent md:rounded-3xl">
        <div className="relative z-10 flex min-h-[70vh] items-center justify-center px-6 py-10 text-center sm:px-10 sm:py-14">
          <div className="-translate-y-8 max-w-xl sm:-translate-y-10">
            <p
              className={`fade-up ${heroBodyFont.className} -mt-6 mb-3 text-xs uppercase tracking-[0.24em] text-slate-600 sm:-mt-8 sm:text-sm`}
            >
              Нумо розвивати простір українськомовних ролівок разом
            </p>
            <h1
              className="fade-up mb-4 text-4xl font-bold leading-[0.95] tracking-tight text-slate-950 sm:text-5xl lg:text-6xl"
            >
              UaRP — пошук рольових і співролів
            </h1>
            <p
              className={`fade-up ${heroBodyFont.className} mt-8 mb-6 max-w-xl text-base font-medium leading-relaxed text-slate-700 sm:mt-10 sm:text-lg`}
            >
